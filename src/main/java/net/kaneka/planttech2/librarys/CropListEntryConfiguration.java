...
public class CropListEntryConfiguration {
	...
	public CropListEntryConfiguration(boolean enabled, EnumTemperature temperature, List<Item> additional_seeds,
			List<Triple<Item, Integer, Integer>> additional_drops, List<Parents> parents, Block soil) {
	...
	public static class Deserializer implements JsonDeserializer<CropListEntryConfiguration> {
		...
		@Override
		public CropListEntryConfiguration deserialize(JsonElement json, Type typeOfT,
			...
			for (JsonElement element : jsonObject.get("additional_drops").getAsJsonArray()) {
				...
				if (object.has("item")) {
					item = ForgeRegistries.ITEMS
							.getValue(new ResourceLocation(element.getAsJsonObject().get("item").getAsString()));
				} else if (object.has("block")) {
					item = ForgeRegistries.ITEMS
							.getValue(new ResourceLocation(element.getAsJsonObject().get("block").getAsString()));
				}

				if (item != null) {
					additional_drops.add(new ImmutableTriple<Item, Integer, Integer>(item, object.get("min").getAsInt(),
			...
			return new CropListEntryConfiguration(enabled, temperature, additional_seeds, additional_drops, parents,
					soil);
		}
	}
}
