...
public class CableModel implements IUnbakedModel
{
    ...
    @Override
    public IBakedModel bake(ModelBakery bakery, Function<ResourceLocation, TextureAtlasSprite> modelGetter, ISprite sprite, VertexFormat format)
    {
	try
	{
	    IUnbakedModel subComponent = ModelLoaderRegistry.getModel(MODEL_CORE);

	    IBakedModel bakedModelCore = subComponent.bake(bakery, modelGetter, sprite, format);

	    IBakedModel[] bakedModelUp = new IBakedModel[4];
	    subComponent = ModelLoaderRegistry.getModel(MODEL_UP_CABLE);
	    bakedModelUp[0] = subComponent.bake(bakery, modelGetter, sprite, format);

	    subComponent = ModelLoaderRegistry.getModel(MODEL_UP_IN);
	    bakedModelUp[1] = subComponent.bake(bakery, modelGetter, sprite, format);

	    subComponent = ModelLoaderRegistry.getModel(MODEL_UP_OUT);
	    bakedModelUp[2] = subComponent.bake(bakery, modelGetter, sprite, format);

	    subComponent = ModelLoaderRegistry.getModel(MODEL_UP_BOTH);
	    bakedModelUp[3] = subComponent.bake(bakery, modelGetter, sprite, format);

	    IBakedModel[] bakedModelDown = new IBakedModel[4];
	    subComponent = ModelLoaderRegistry.getModel(MODEL_DOWN_CABLE);
	    bakedModelDown[0] = subComponent.bake(bakery, modelGetter, sprite, format);

	    subComponent = ModelLoaderRegistry.getModel(MODEL_DOWN_IN);
	    bakedModelDown[1] = subComponent.bake(bakery, modelGetter, sprite, format);

	    subComponent = ModelLoaderRegistry.getModel(MODEL_DOWN_OUT);
	    bakedModelDown[2] = subComponent.bake(bakery, modelGetter, sprite, format);

	    subComponent = ModelLoaderRegistry.getModel(MODEL_DOWN_BOTH);
	    bakedModelDown[3] = subComponent.bake(bakery, modelGetter, sprite, format);

	    IBakedModel[] bakedModelEast = new IBakedModel[4];
	    subComponent = ModelLoaderRegistry.getModel(MODEL_EAST_CABLE);
	    bakedModelEast[0] = subComponent.bake(bakery, modelGetter, sprite, format);

	    subComponent = ModelLoaderRegistry.getModel(MODEL_EAST_IN);
	    bakedModelEast[1] = subComponent.bake(bakery, modelGetter, sprite, format);

	    subComponent = ModelLoaderRegistry.getModel(MODEL_EAST_OUT);
	    bakedModelEast[2] = subComponent.bake(bakery, modelGetter, sprite, format);

	    subComponent = ModelLoaderRegistry.getModel(MODEL_EAST_BOTH);
	    bakedModelEast[3] = subComponent.bake(bakery, modelGetter, sprite, format);

	    IBakedModel[] bakedModelWest = new IBakedModel[4];
	    subComponent = ModelLoaderRegistry.getModel(MODEL_WEST_CABLE);
	    bakedModelWest[0] = subComponent.bake(bakery, modelGetter, sprite, format);

	    subComponent = ModelLoaderRegistry.getModel(MODEL_WEST_IN);
	    bakedModelWest[1] = subComponent.bake(bakery, modelGetter, sprite, format);

	    subComponent = ModelLoaderRegistry.getModel(MODEL_WEST_OUT);
	    bakedModelWest[2] = subComponent.bake(bakery, modelGetter, sprite, format);

	    subComponent = ModelLoaderRegistry.getModel(MODEL_WEST_BOTH);
	    bakedModelWest[3] = subComponent.bake(bakery, modelGetter, sprite, format);

	    IBakedModel[] bakedModelNorth = new IBakedModel[4];
	    subComponent = ModelLoaderRegistry.getModel(MODEL_NORTH_CABLE);
	    bakedModelNorth[0] = subComponent.bake(bakery, modelGetter, sprite, format);

	    subComponent = ModelLoaderRegistry.getModel(MODEL_NORTH_IN);
	    bakedModelNorth[1] = subComponent.bake(bakery, modelGetter, sprite, format);

	    subComponent = ModelLoaderRegistry.getModel(MODEL_NORTH_OUT);
	    bakedModelNorth[2] = subComponent.bake(bakery, modelGetter, sprite, format);

	    subComponent = ModelLoaderRegistry.getModel(MODEL_NORTH_BOTH);
	    bakedModelNorth[3] = subComponent.bake(bakery, modelGetter, sprite, format);

	    IBakedModel[] bakedModelSouth = new IBakedModel[4];
	    subComponent = ModelLoaderRegistry.getModel(MODEL_SOUTH_CABLE);
	    bakedModelSouth[0] = subComponent.bake(bakery, modelGetter, sprite, format);

	    subComponent = ModelLoaderRegistry.getModel(MODEL_SOUTH_IN);
	    bakedModelSouth[1] = subComponent.bake(bakery, modelGetter, sprite, format);

	    subComponent = ModelLoaderRegistry.getModel(MODEL_SOUTH_OUT);
	    bakedModelSouth[2] = subComponent.bake(bakery, modelGetter, sprite, format);

	    subComponent = ModelLoaderRegistry.getModel(MODEL_SOUTH_BOTH);
	    bakedModelSouth[3] = subComponent.bake(bakery, modelGetter, sprite, format);
	...
	catch (Exception exception)
	{
	    ...
	    return ModelLoaderRegistry.getMissingModel().bake(bakery, modelGetter, sprite, format);
	}
    }
