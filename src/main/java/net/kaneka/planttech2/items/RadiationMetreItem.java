...
//            player.sendMessage(new TextComponent("Your Radiation Level: " + new TextComponent(new DecimalFormat("###.########").format(level)).mergeStyle(Style.EMPTY.setColor(Color.fromTextFormatting(level <= 0.20F ? TextFormatting.BLUE : level <= 0.40F ? TextFormatting.AQUA : level <= 0.60F ? TextFormatting.GREEN : level <= 0.80F ? TextFormatting.YELLOW : TextFormatting.RED)))), player.getUniqueID());
//            Biome biome = worldIn.getBiome(player.getPosition());
//            player.sendMessage(new TextComponent("Biome Radiation Level: " + BiomeRadiation.getText(biome instanceof PlantTopiaBaseBiome ? ((PlantTopiaBaseBiome) biome).getRadiationLevel() : BiomeRadiation.NONE, true).getFormattedText()), player.getUniqueID());
//        }
